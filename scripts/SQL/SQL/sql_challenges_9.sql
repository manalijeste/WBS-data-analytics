...
FROM sales s LEFT JOIN titles t
...
FROM employee e LEFT JOIN publishers p
...
FROM employee e LEFT JOIN jobs j
ON e.job_id = j.job_id
LEFT JOIN publishers p ON p.pub_id = e.pub_id
WHERE p.pub_name = "Binnet & Hardley";

/* 8. Select the name and PR Info (from the pub_info table) from all publishers
   based in Berkeley, California. */

SELECT pi.pr_info, p.pub_name, p.city
FROM pub_info pi LEFT JOIN publishers p
...
SELECT *
FROM stores s LEFT JOIN discounts d
...
SELECT s.stor_name, d.discounttype, d.discount
